...
// 1. Criamos a Interface para os dados do utilizador
...
// 2. NavLink do lado de fora para evitar re-renderizações desnecessárias
...
export function Sidebar() {
  ...
  return (
    <>
      <aside className="w-64 h-full bg-[#111827] border-r border-[#1F2937] flex flex-col shadow-2xl relative z-10">
        ...
        <div className="px-4 mt-2">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-red-500/20 transition-all hover:scale-[1.02]"
          >
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1.5">
          ...
          {usuario?.tipo_conta === 'pessoal' && (
            <div className="mt-8 mx-2 p-4 bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl relative overflow-hidden animate-fade-in group">
              ...
              <button 
                onClick={() => setMostrarPlanos(true)}
                className="w-full py-2 bg-slate-800 hover:bg-red-500 text-white text-xs font-bold rounded-lg border border-slate-700 hover:border-red-500 transition-all shadow-lg relative z-10"
              >
                Fazer Upgrade
              </button>
            </div>
      ...
      {mostrarPlanos && <UpgradeModal onClose={() => setMostrarPlanos(false)} />}
      
      {/* Renderização Condicional do Modal usando Portal */}
      {mostrarPlanos && createPortal(
        <UpgradeModal onClose={() => setMostrarPlanos(false)} />,
        document.body
      )}
    </>
  );
